...
import ProfileHeader, {
  type ProfileHeaderTeamMember,
} from "@/components/team-member-profile/ProfileHeader";
import AchievementsGrid from "@/components/team-member-profile/AchievementsGrid";
import PortfolioGallery, {
  type PortfolioItem,
} from "@/components/team-member-profile/PortfolioGallery";
import EditProfileModal from "@/components/team-member-profile/EditProfileModal";
import LandingPageEditor from "@/components/team-member-profile/LandingPageEditor";
import {
  coerceLandingConfig,
...
import {
  ...
  type TeamMemberProfileExtras,
} from "@/lib/gamification";

type TeamMemberRow = ProfileHeaderTeamMember & {
  id: string;
  bio: string | null;
  email: string;
  phone?: string | null;
...
const EMPTY_STATS: TeamMemberStats = {
  wonCount: 0,
  activeCount: 0,
  totalCustomers: 0,
  tasksCompleted: 0,
...
const EMPTY_EXTRAS: TeamMemberProfileExtras = {
  gotoConnected: false,
...
export default function TeamMemberProfilePage({
  ...
  const [extras, setExtras] = useState<TeamMemberProfileExtras>(EMPTY_EXTRAS);
  const [stats, setStats] = useState<TeamMemberStats>(EMPTY_STATS);
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      // Fetch all active team members and match by email handle (local part before @).
      // TeamMember list is small so client-side match is fine and handles duplicates.
      // We try to select profile columns added by the team member-profiles migration.
      ...
      // Try UUID match first (new links use teamMember.id), then fall back to
      ...
      const match = (rows ?? []).find((b) => {
        ...
        return teamMemberHandle(row.email) === handle;
    ...
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center px-4 text-center">
        ...
        <p className="mt-1 text-sm text-gray-500">
          No team member with the handle &ldquo;{handle}&rdquo; was found.
        </p>
        <Link
  ...
  const completenessSteps = getCompletenessSteps(teamMember, extras);
  const xp = computeXp(stats, completenessSteps);
  const level = getLevelProgress(xp);
  const checklist = getProfileChecklist(teamMember, stats.portfolioCount, extras);
  const strength = getProfileStrength(checklist);
  const achievements = getAchievements(stats, teamMember, extras, strength);
  const isOwn = currentUserId === teamMember.id;
  const landingConfig: LandingConfig = coerceLandingConfig(teamMember.landing_config);
  const landingStatus = teamMember.landing_status ?? "draft";
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6">
      ...
      {isOwn && teamMember.profile_slug && (
        <div className="mb-4 flex items-center gap-2 rounded-lg border border-[#E85D04]/30 bg-[#E85D04]/5 px-4 py-2.5">
          ...
          <span className="text-sm text-gray-600">
            Your public page:
            <a
              href={`/rep/${teamMember.profile_slug}`}
              target="_blank"
              rel="noopener noreferrer"
              className="ml-1 font-medium text-[#E85D04] hover:underline"
            >
              /rep/{teamMember.profile_slug}
            </a>
          </span>
          <button
            onClick={() => {
              navigator.clipboard.writeText(
                `${window.location.origin}/rep/${teamMember.profile_slug}`,
              );
            }}
      ...
      {showLandingEditor && (
        <LandingPageEditor
          ...
          reviewNote={teamMember.landing_review_note ?? null}
          profileSlug={teamMember.profile_slug ?? null}
