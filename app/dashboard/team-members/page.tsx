...
type TeamMemberCard = {
  ...
  // Profile columns added by the team member-profiles migration — may be null/absent
...
export default function TeamMemberDirectoryPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setTeamMembers((data as TeamMemberCard[] | null) ?? []);
      setLoading(false);
    }
    load();
  }, [supabase]);

  const filtered = teamMembers.filter((b) => {
  ...
  const grouped: Record<string, TeamMemberCard[]> = {};
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Meet the teamMembers at Nationwide Transport Services
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="py-20 text-center text-gray-400">No team members found.</div>
      ) : (
            ...
            <div key={office}>
              <div className="mb-4 flex items-center gap-3">
                ...
                <span className="text-sm text-gray-400">
                  {grouped[office].length} {grouped[office].length === 1 ? "teamMember" : "teamMembers"}
                </span>
                <div className="h-px flex-1 bg-gray-100" />
              </div>
              <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4">
                {grouped[office].map((teamMember) => {
                  const fullName =
                  ...
                  return (
                    <Link
                      key={teamMember.id}
                      href={`/dashboard/team-members/${teamMember.id}`}
                    ...
                    >
                      ...
                      <div className="p-3">
                        ...
                        <p className="mt-0.5 text-xs uppercase tracking-wide text-gray-400">
                          {teamMember.headline ?? "Logistics Agent"}
                        </p>

                        {teamMember.specialties && teamMember.specialties.length > 0 && (
                          <div className="mt-2 flex flex-wrap gap-1">
                            {teamMember.specialties.slice(0, 2).map((s) => (
                              <span
                                key={s}
