...
export async function POST(request: Request) {
  try {
    ...
    const {
      ...
      office,
      isRemote,
      // The Admin UI still sends boolean flags. Accept an explicit `role`
      // enum too so newer callers can be unambiguous. Booleans win only
      // when role is not provided.
      isAdmin,
      isManager,
      role: roleInput,
    }: {
    ...
    // Authorize: caller must be an active admin (profiles.role = 'admin').
    ...
    const { data: callerUser } = await supabaseAdmin.auth.getUser(token);
    if (!callerUser?.user) {
      return NextResponse.json(
    ...
    // Resolve the role enum from explicit input or the legacy booleans.
    const allowedRoles = ["admin", "manager", "claims_staff", "broker"] as const;
    type RoleEnum = (typeof allowedRoles)[number];
    let resolvedRole: RoleEnum;
    if (roleInput && (allowedRoles as readonly string[]).includes(roleInput)) {
      resolvedRole = roleInput as RoleEnum;
    } else if (isAdmin) {
      resolvedRole = "admin";
    } else if (isManager) {
      resolvedRole = "manager";
    } else {
      resolvedRole = "claims_staff";
    }
    ...
    // The `handle_new_auth_user` trigger auto-creates the profiles row. We
    // upsert it here to (a) cover the existing-user case where no insert
    // happened and (b) apply the requested role + office details.
    ...
    // Create welcome notification (notifications.user_id, not team_member_id)
    ...
    try {
      ...
      if (!emailSent) {
        ...
        return NextResponse.json({
          success: true,
          warning:
            "TeamMember created but welcome email failed to send. Use 'Resend Invite' to try again.",
    ...
    return NextResponse.json({
      success: true,
      message: "TeamMember invited successfully. Welcome email sent.",
    });
  } catch (error) {
