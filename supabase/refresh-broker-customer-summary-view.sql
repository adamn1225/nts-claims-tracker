-- Refresh broker_customer_summary view to properly reflect first_name/last_name changes
-- The view was using an old definition that concatenated into full_name
...
SELECT 
  ...
  SUM(CASE WHEN c.status = 'prospect' THEN 1 ELSE 0 END) as prospects,
...
COMMENT ON VIEW broker_customer_summary IS 'Summary view of brokers with their customer statistics. Now includes first_name and last_name separately plus full_name computed field.';
