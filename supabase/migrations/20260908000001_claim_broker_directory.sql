-- Broker / logistics-agent reporting support, built on the existing
-- team_members table. claims.team_member_id already points at team_members
-- (renamed from broker_id in 20260621000001) — this migration re-purposes
-- that existing relationship rather than adding a new table, and exposes a
-- safe public subset for the anonymous claim intake form.

-- Anonymous-safe view: only non-PII columns, active brokers only. Views are
-- definer-scoped by default (no security_invoker), so this bypasses RLS on
-- team_members using the migration role's privileges — the column allowlist
-- below is the actual security boundary, not RLS.
create view public.broker_directory
  with (security_invoker = false)
as
select
  id,
  trim(coalesce(nullif(trim(display_name), ''), first_name || ' ' || last_name)) as name,
  office_location
from public.team_members
where is_active = true;

comment on view public.broker_directory is
  'Public-safe subset of team_members (id, name, office_location) for the anonymous claim intake form''s Logistics Representative selector. Does not expose email, phone, or other team_members columns.';

grant select on public.broker_directory to anon, authenticated;

comment on column public.claims.team_member_id is
  'Broker / logistics agent (from team_members) assigned to the CRM order that became this claim. Renamed from broker_id in migration 20260621000001; re-purposed here for claims reporting/filtering.';

-- Extend the claim-detail audit trigger to also track broker reassignment.
...
drop trigger if exists claims_detail_audit on public.claims;
