...
export default function EditBrokerModal({
  ...
  useEffect(() => {
    if (broker) {
      ...
      setShowInDirectory(broker.show_in_directory ?? true);
      setError(null);
      setSuccess(false);
    }
  }, [broker, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmitAction({
        ...
        is_admin: isAdmin ? is_admin : (broker.is_admin ?? false),
        is_manager: isAdmin ? is_manager : (broker.is_manager ?? false),
        show_in_directory,
      });
      setSuccess(true);
      setTimeout(() => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-lg sm:p-8">
        ...
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-900">Edit Broker</h2>
          <button
